...
buses = [
    {"bus_no": "MH101", "source": "Pune", "destination": "Mumbai", "price": 550},
    {"bus_no": "MH202", "source": "Mumbai", "destination": "Nashik", "price": 450},
    {"bus_no": "MH303", "source": "Nagpur", "destination": "Aurangabad", "price": 800},
    {"bus_no": "MH404", "source": "Kolhapur", "destination": "Pune", "price": 400},
    {"bus_no": "MH505", "source": "Solapur", "destination": "Nagpur", "price": 900},
    {"bus_no": "MH606", "source": "Nashik", "destination": "Kolhapur", "price": 650},
]
bookings = []
def style_app():
    style = ttk.Style()
    ...
    style.configure("Header.TLabel", background="snow", foreground="darkred", font=("Arial", 16, "bold"))
    ...
    style.configure("Accent.TButton", background="darkred", foreground="white", font=("Arial", 10, "bold"), padding=6)
    style.map("Accent.TButton", background=[("active", "red")], foreground=[("active", "snow")])

class LoginPage:
    def __init__(self, root):
        self.root = root
        self.root.title("Red Bus - Login / Register")
        ...
        ttk.Label(frame, text="Red Bus", style="Header.TLabel").pack(pady=15)
...
class Dashboard:
    def __init__(self, user):
        self.user = user
        self.root = tk.Tk()
        self.root.title(f"Red Bus - Welcome {user}")
        ...
        src_list = ["All"] + sorted(set([b["source"] for b in buses]))
        dest_list = ["All"] + sorted(set([b["destination"] for b in buses]))
        ...
        ttk.Button(filter_frame, text="Apply Filter", style="Accent.TButton", command=self.refresh_buses).grid(row=0, column=4, padx=5)

        self.tree = ttk.Treeview(self.root, columns=("Bus", "Source", "Destination", "Price"), show="headings", height=12)
        for col in ("Bus", "Source", "Destination", "Price"):
        ...
        ttk.Button(btn_frame, text="Book Selected Bus", style="Accent.TButton", command=self.book_bus).grid(row=0, column=0, padx=10)
    ...
    def refresh_buses(self):
        ...
        for b in buses:
            if (src_filter == "All" or b["source"] == src_filter) and (dest_filter == "All" or b["destination"] == dest_filter):
                self.tree.insert("", "end", values=(b["bus_no"], b["source"], b["destination"], f"₹{b['price']}"))

    def book_bus(self):
        selected = self.tree.focus()
        if not selected:
            messagebox.showwarning("Warning", "Select a bus to book!")
            return
    ...
    def show_booking_popup(self, booking):
        ...
        ttk.Label(win, text=f"Bus No: {booking['bus_no']}", background="snow").pack(pady=5)
    ...
    def show_bookings(self):
        ...
        tree = ttk.Treeview(win, columns=("ID", "Bus", "Source", "Destination", "Tickets", "Total"), show="headings", height=10)
        for col in ("ID", "Bus", "Source", "Destination", "Tickets", "Total"):
            tree.heading(col, text=col)
            tree.column(col, anchor="center", width=110)
        tree.pack(padx=20, pady=10, fill="both")
        for b in bookings:
            if b["user"] == self.user:
                tree.insert("", "end", values=(b["id"], b["bus_no"], b["source"], b["destination"], b["tickets"], f"₹{b['total']}"))
