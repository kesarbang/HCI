...
class Dashboard:
    def __init__(self, user):
        ...
        self.tree = ttk.Treeview(self.root, columns=("Flight", "Source", "Destination", "Price", "Seats"), show="headings", height=12)
        for col in ("Flight", "Source", "Destination", "Price", "Seats"):
    ...
    def refresh_flights(self):
        ...
        for f in flights:
            if (src_filter == "All" or f["source"] == src_filter) and (dest_filter == "All" or f["destination"] == dest_filter):
                if f["seats"] > 0:
                    self.tree.insert("", "end", values=(f["flight_no"], f["source"], f["destination"], f"₹{f['price']}", f["seats"]))
    ...
    def show_booking_popup(self, booking):
        ...
        ttk.Label(win, text=f"Fare: ₹{booking['price']}", background="snow").pack(pady=5)
    ...
    def show_bookings(self):
        ...
        tree = ttk.Treeview(win, columns=("ID", "Flight", "Source", "Destination", "Price"), show="headings", height=10)
        for col in ("ID", "Flight", "Source", "Destination", "Price"):
            tree.heading(col, text=col)
            tree.column(col, anchor="center", width=120)
        tree.pack(padx=20, pady=10, fill="both")
        for b in bookings:
            if b["user"] == self.user:
                tree.insert("", "end", values=(b["id"], b["flight_no"], b["source"], b["destination"], f"₹{b['price']}"))
